	...
	if tunnel == 'telnet':
		try:
			...
			while True:
				...
				expectedoutput = ['(?i)Authentication failed', '(?i)Access denied', '(?i)Invalid input',
								  '(?i)Username:', '(?i)Password:', '(?i)' + hostname,
								  '(?i)' + hostname + ' [(]enable[)]', '(?i)' + hostname + '#', '>']
				...
				if expectresult[0] == -1:
					...
					sock.write(user.encode('ascii') + b'\n')
				elif expectresult[0] == 4:
					sock.write(pw.encode('ascii') + b'\n')
					pwgiven = True
				elif pwgiven:
					if expectresult[0] != 0:
						break  # escapes loop
			...
			sock.write(en_pw.encode('ascii') + b'\n')
		...
		try:
			sock.connect(connectionname, 22, user, pw)
		except:
			addtoreportdata(region, connectionname, hostname, 'AUTHENTICATION FAILED', reportlist, errorlist)
			sock.close()
			return
